...
<shiro:hasPermission name="sys:manager:updateStatus">
...
<shiro:hasPermission name="sys:manager:roleUpdate">
    <input id="p_role_update" style="display: none" value="1">
</shiro:hasPermission>
<shiro:hasPermission name="sys:manager:resetPwd">
    <input id="p_resetPwd" style="display: none" value="1">
</shiro:hasPermission>
<shiro:hasPermission name="sys:manager:region:edit">
    <input id="p_region_update" style="display: none" value="1">
</shiro:hasPermission>
<div class="row">
    <div class="col-lg-12">
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table id="myTable"
                           ...
                           data-url="<%=request.getContextPath()%>/admin/manager/json"
                        ...
                        <tr>
                            ...
                            <th width="5%" data-field="loginCount">登陆次数</th>
...
<script type="text/javascript">
    ...
    function tableCallBack(data, id){
        if (id == 'sex') {
            ...
            if ($('#p_region_update').val() == 1) {
                _button += '<button type="button" class="btn btn-primary btn-xs" onclick="editUserRegion(' + data.id + ');"> 修改区域 </button> ';
                count ++
            }

            if ($('#p_resetPwd').val() == 1) {
                _button += '<button type="button" class="btn btn-warning btn-xs" onclick="resetPwd(' + data.id + ');"> 重置密码 </button> ';
                count ++
            }
            ...
            if ($('#p_delete').val() == 1) {
                _button += '<button type="button" class="btn btn-danger btn-xs" onclick="deleteUser(' + data.id + ');">删除</button> ';
                count ++
            }
    ...
    function addUser(){
        layer.open({
            ...
            title: '添加后台用户',
            content: ['<%=request.getContextPath()%>/admin/manager/create','no']
        });
    }
    ...
    function editUser(id){
        layer.open({
            ...
            offset: [100,(($(window).width()-900)/2)],
            title: '修改后台用户信息',
            content: ['<%=request.getContextPath()%>/admin/manager/update?id=' + id,'no']
        });
    }
    ...
    function deleteUser(id){
        layer.confirm('是否确定此操作？', {
            ...
            $.ajax({
                type: 'POST',
                url: '<%=request.getContextPath()%>/admin/manager/delete?id=' + id,
                dataType: 'json',
                success: function (data) {
                    if (data.status == 'ok') {
                        layer.alert('删除成功!', {
    ...
    function editUserRegion(id){
        layer.open({
            type: 2,
            area: ['900px', '600px'],
            offset: [100,(($(window).width()-900)/2)],
            title: '分配后台用户角色权限',
            content: ['<%=request.getContextPath()%>/admin/region/user?id=' + id,'no']
        });
    }
    ...
    function editStatus(id, state){
        ...
        layer.confirm('是否确定此操作？', {
            ...
            $.ajax({
                url: '<%=request.getContextPath()%>/admin/manager/updateStatus',
                dataType: 'json',
                type: 'POST',
                data: {
                    id:id,
