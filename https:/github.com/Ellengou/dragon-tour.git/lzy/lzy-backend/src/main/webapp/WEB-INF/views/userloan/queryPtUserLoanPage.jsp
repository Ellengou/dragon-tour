...
<body>
<shiro:hasPermission name="userloan:to:loan">
    <input id="p_toloan" style="display: none" value="1">
</shiro:hasPermission>
	<div class="row">
		<div class="col-lg-12">
				...
				<div class="panel-body">
					<div class="table-responsive">
						<table id="myTable"
							...
							data-url="<%=request.getContextPath()%>/admin/userloan/queryPtUserLoan"
	...
	function tableCallBack(data,id){
        if(id == 'button'){
        	var str = '';
        	str += '<button type="button" class="btn btn-primary btn-xs" onclick="queryUserLoan(\'' + data.id + '\');">查看</button>';
        	if($("#p_toloan").val() == 1){
	        	if(data.loanStatus == 'apply_success' || data.loanStatus == 'waiting_give_money' || data.loanStatus == 'repaying'){
	        		if(data.restLoanMoney == 0){
		        		str += '<button type="button" class="btn btn-success btn-xs">额度已发完</button>';
	        		}else{
	        			str += '<button type="button" class="btn btn-primary btn-xs" onclick="addLoan(\'' + data.id + '\');">发标</button>';
	        		}
	        	}
        	}
            return str;
	...
	function addLoan(id){
		var index = layer.open({
	        ...
	        title: false,
	        content: '<%=request.getContextPath()%>/admin/project/loan/addLoanFromDebtPage?id=' + id
	    });
		layer.full(index);
	}
