 ...
 */
  ...
  */
	...
	$.validator.addMethod("telePhone", function (value, element) {
		var telephoneVerity = /^1[3|4|5|7|8][0-9]\d{8}$/;
		return this.optional(element) || telephoneVerity.test(value);
	...
	//模拟下拉列表select-btn
	$.validator.addMethod("selectBtn", function (value, element) {
		var selectVal = "请选择！";
		var flage = true;
		var sel = $('input[name="' + element.name + '"]');
		//链接验证 选择后 验证提示隐藏
		var selWrap = sel.closest(".select-wrapper");
		selWrap.on("click","ul li",function(){
			var selLi = $(this);
			var selWrapId =  selWrap.attr("id");
			if(sel.closest(".form-row").find('#'+selWrapId+'-error').length>0 && $.trim(selLi.data('value')) !=""){
				$(this).closest(".form-row").find('#'+selWrapId+'-error').hide().html("");
			}
			else{
				$(this).closest(".form-row").find('#'+selWrapId+'-error').show().html(selectVal);
				sel.closest('.select-wrapper').addClass('focus');
			}
		});
		if($.trim(sel.val()) !== ""){
			flage = true;
	...
	//模拟下拉列表 地区选择必须选 省 市 区  都要areaSelect验证
	$.validator.addMethod("areaSelect", function (value, element) {
		var selectVal = "全部选择！";
		var flage = true;
		var sel = $('input[name="' + element.name + '"]');
		//链接验证 选择后 验证提示隐藏
		...
		selWrap.on("click","ul li",function(){
			var selLi = $(this);
			//var selWrapId =  selWrap.attr("id");
			if($.trim(selLi.data('value')) !=""){
				if(sel.closest(".select-wrapper").hasClass('district')){
					$(this).closest(".form-row").find('span.error').hide().html("");
				}
			}
			else{
				$(this).closest(".form-row").find('span.error').show().html(selectVal);
				sel.closest('.select-wrapper').addClass('focus');
			}
		});
		if($.trim(sel.val()) !== ""){
			...
			sel.closest('.select-wrapper').addClass('focus');
			var wrapers =  sel.closest('form').find("div.focus");
			if(wrapers.length>1){
				wrapers.not(":first").removeClass("focus");
			}
			flage = false;
		}
		return flage;
	...
	$.validator.addMethod("checkOne", function (value, element) {
		var sel = $('input[name="' + element.name + '"]');
		var moreChecks = sel.closest('.input-group').find("input[type='checkbox']:checked");
		if (moreChecks.length) {
			return true;
		} else {
			return false;
		}
	...
	$.validator.addMethod("isQq", function(value, element) {
		var qqTest = /^[1-9]\d{4,12}$/;
		return this.optional(element) || qqTest.test(value);
	}, "匹配QQ");

	//银行卡号验证
	$.validator.addMethod("isBankNo", function(value, element) {
		var bankNo = /^[0-9]{15,19}$/g;
		return this.optional(element) || bankNo.test(value);
	}, "请输入正确的银行卡号。");


	//月收入金额验证
	$.validator.addMethod("isIncome", function(value, element) {
		var inComeDot = /^\d+(\.\d{2})+$/;
		var valueLen = value.length;
		return this.optional(element) || (valueLen <= 9 && $.validator.methods.digits.call(this, value, element))|| (valueLen <= 12 && inComeDot.test(value));
	}, "请输入正确的金额。月收入不能超过亿元");
