...
<body>
	<shiro:hasPermission name="empSalary:grant">
	    <input id="p_grant" style="display: none" value="1">
	</shiro:hasPermission>
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				...
				<div class="panel-heading">
					<a class="btn btn-primary" href="<%=request.getContextPath()%>/template/员工工资.xls" target="_self">员工工资模板下载</a> 
					<shiro:hasPermission name="empSalary:upload">
						<button type="button" class="btn btn-primary" onclick="uopload();">上传员工工资</button>
					</shiro:hasPermission>
					<shiro:hasPermission name="empSalary:grant">
						<button type="button" class="btn btn-primary" onclick="grantAll();">申请发放</button>
					</shiro:hasPermission>
				...
				<div class="panel-body">
					<div class="table-responsive">
						<table id="myTable"
							...
							data-url="<%=request.getContextPath()%>/admin/empSalary/queryGrantEmpSalarys"
	...
	$(function(){
		//var salaryGrantStatusList = $.parseJSON('${salaryGrantStatusList}');
		//for (var i = 0; i < salaryGrantStatusList.length; i++) {
		//	$(new Option(salaryGrantStatusList[i].dicDataName,salaryGrantStatusList[i].dicDataValue)).appendTo('#status');
		//}
	...
	function tableCallBack(data,id){
    	if(id == 'status'){
    		...
    		return data[id];
   			...
   			if($('#p_grant').val() == 1){
   				return '<button type="button" class="btn btn-primary btn-xs" onclick="grant(\'' + data.id + '\');">申请发放</button> ';
    		}else{
    			return '';
    		}
    	}else{
	...
	function uopload(){
		layer.open({
    	    ...
    	    title: '上传员工工资页面',
    	    content: ['<%=request.getContextPath()%>/admin/empSalary/uploadSysEmpSalaryPage','no']
    	});
	}
	...
	function grantAll(){
        ...
        if(chk_value.join(",") == null || chk_value.join(",") == ''){
            layer.alert('请选择需要发放的工资', {icon : 5});
            return;
        }
        grant(chk_value.join(","));
	}
	...
	function grant(id){
		...
		layer.confirm('是否确定此操作？', {
			...
			$.ajax({
			      url: '<%=request.getContextPath()%>/admin/empSalary/grantSysEmpSalary',
			      dataType: 'json',
			      type: 'POST',
			      data: {
			    	  ids: id
