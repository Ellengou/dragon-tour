...
<body>
				...
				<div class="panel-body">
					<div class="table-responsive">
						<table id="myTable"
							...
							data-url="<%=request.getContextPath()%>/admin/userloan/queryQyHkzUserLoan"
	...
	$("#moneySource").showList('option', {
		infoList : '${moneySourceList}'
	});
	function tableCallBack(data,id){
        if(id == 'button'){
        	var str = '';
            str += '<button type="button" class="btn btn-primary btn-xs" onclick="queryUserLoan(\'' + data.id + '\');">查看</button>';;
        	str += '<button type="button" class="btn btn-primary btn-xs" onclick="queryRepayInfo(\'' + data.id + '\');">还款计划</button>';
        	return str;
	...
	function queryRepayInfo(id){
		var index = layer.open({
            type: 2,
            offset: [100,(($(window).width()-900)/2)],
            title: false,
            content: '<%=request.getContextPath()%>/admin/userloan/queryRepayInfoPage?id=' + id
        });
		layer.full(index);
	}
