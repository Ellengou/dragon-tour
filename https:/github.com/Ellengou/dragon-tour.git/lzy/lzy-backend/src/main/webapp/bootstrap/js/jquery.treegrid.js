...
(function($) {
    ...
    var methods = {
            ...
            if (options.data) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var template = initChildNode(data);
                        console.log("")
                        $(".tree tbody").html(template);

                        var settings = $.extend({}, tmp.treegrid.defaults, options);
                        return tmp.each(function () {
                            var $this = $(tmp);
                            $this.treegrid('setTreeContainer', $(tmp));
                            $this.treegrid('setSettings', settings);
                            settings.getRootNodes.apply(tmp, [$(tmp)]).treegrid('initNode', settings);
                        });
                    }
                });
            }
        ...
        /**
         * add something new function
         *
         * @author violorch
         * **/
        ifSelectOne: function () {
            return $(this).treegrid('getSetting', 'ifSelectOne');
        },
        removeCss: function () {
            return $(this).treegrid('getSetting', 'removeCss');
        },
        getSelectData: function () {
            var _selectRows = new Array();
            var _selectRowsCount = 0;
            $(".tree tr").each(function(){
                ...
                if (tr.attr('check') === 'T') {
                    ...
                    var tds = tr.find("td");
                    var _tdData = new Array();
                    tds.each(function () {
                        var td = $(this);
                        _tdData[_tdCount] = td.text();
                        _tdCount ++;
                    });
                    _selectRows[_selectRowsCount] = _tdData;
                    _selectRowsCount ++;
                }
            });
    ...
    $.fn.treegrid.defaults = {
        ...
        expanderTemplate: '<span class="treegrid-expander" onclick="expanderClick(event)"></span>',
...
function initChildNode(data) {
    $('.tree tbody').html('');
    var template = "";
    for(var i = 0; i < data.length; i++){
        var listCount = 0;
        if (data[i].pid == 0) {
            template += '<tr id="' + data[i].id + '" onclick="onSelectNode($(this), ' + data[i].id + ');" check="F" class="treegrid-' + data[i].id + '">';
        } else {
            template += '<tr id="' + data[i].id + '" onclick="onSelectNode($(this), ' + data[i].id + ');" check="F" class="treegrid-' + data[i].id + ' treegrid-parent-' + data[i].pid + '">';
        }
        $(".tree").find("th").each(function(){
            if ($(this).attr('data-call') == 'true') {
                template += '<td class="hidden-xs">' + tableCallBack(data[i], $(this).attr('data-field')) + '</td>';
            } else {
                template += '<td class="hidden-xs">' + data[i][$(this).attr('data-field')] + '</td>';
        ...
        if (data[i].children.length > 0) {
            template += initChildNode(data[i].children);
        }
    }
    return template;
}
...
function onSelectNode(node, data) {
    if ($('.tree').treegrid('removeCss')) return;
    var ifCheck = node.attr('check');
    if ($('.tree').treegrid('ifSelectOne')) {
        $(".tree tr").each(function () {
            setUnCheck($(this));
        });
    }
...
function setCheck(node) {
    ...
    if (!$('.tree').treegrid('ifSelectOne')) {
        ...
        if (_class.indexOf('treegrid-parent') > -1) {
            var _pid = _class.split(' ')[1].match(/\d+\b/);
            var _childNode = '.' + 'treegrid-' + _pid;
            setCheck($(_childNode));
        }
    }
}

function setUnCheck(node) {
    node.attr('check', 'F');
    node.css('background', 'white');
    node.css('color', 'black');
    // 子级渲染
    if (!$('.tree').treegrid('ifSelectOne')) {
        var _pid = node.attr('class').match(/\d+\b/);
        var _classPid = '.treegrid-parent-' + _pid;
        var childNodes = $(_classPid);
        for (var i = 0; i < childNodes.length; i++) {
            var _thisNodeClassName = childNodes[i].className;
            var _thisNodeClassNameMini = '.treegrid-' + _thisNodeClassName.match(/\d+\b/);
            setUnCheck($(_thisNodeClassNameMini));
        }
    }
}
