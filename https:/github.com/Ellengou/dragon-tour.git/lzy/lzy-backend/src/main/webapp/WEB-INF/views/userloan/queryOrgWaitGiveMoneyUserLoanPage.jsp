...
<body>
				...
				<div class="panel-body">
					<div class="table-responsive">
						<table id="myTable"
							...
							data-url="<%=request.getContextPath()%>/admin/userloan/queryOrgWaitGiveMoneyUserLoan"
	...
	function tableCallBack(data,id){
        if(id == 'button'){
        	var str = '';
        	str += '<button type="button" class="btn btn-primary btn-xs" onclick="queryUserLoan(\'' + data.id + '\');">放款</button>';;
        	if(data['dataSource'] == '1'){
	        	str += '<button type="button" class="btn btn-danger btn-xs" onclick="deleteRecord(\'' + data.id + '\');">删除</button>';
        	}
            return str;
        }else{
	...
	function deleteRecord(id){
        ...
        layer.confirm('是否确定删除？', {
        	...
        	$.ajax({ 
    			url : '<%=request.getContextPath()%>/admin/userloan/deleteRecord',
    			type : 'post',
    			dataType: 'json',
    			data : {id : id},
	...
	function queryUserLoan(id){
		var index = layer.open({
	        ...
	        title: false,
	        content: '<%=request.getContextPath()%>/admin/userloan/orgGiveMoney?id=' + id
	    });
		layer.full(index);
	}
