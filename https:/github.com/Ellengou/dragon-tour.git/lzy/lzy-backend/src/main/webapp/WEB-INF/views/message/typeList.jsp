...
<body>
<shiro:hasPermission name="message:type:update">
	<input id="p_update" value="1" type="hidden">
</shiro:hasPermission>
<shiro:hasPermission name="message:type:updateStatus">
	<input id="p_update_status" value="1" type="hidden">
</shiro:hasPermission>
<shiro:hasPermission name="message:type:delete">
	<input id="p_delete" value="1" type="hidden">
</shiro:hasPermission>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<shiro:hasPermission name="message:type:add">
...
<script type="text/javascript">
	$(function() {
		$('#treeGrid').treegrid({
			data:'<%=request.getContextPath()%>/admin/messageType/json?id=0',
	...
	function addType(type) {
		...
		var index = layer.open({
			...
			title: '消息类型管理->新增消息类型',
			content: ['<%=request.getContextPath()%>/admin/messageType/create?pid=' + pid, 'no']
		});
	...
	function editType(id){
		...
		var index = layer.open({
			...
			offset: [100,($(window).width()/5)],
			title: '消息类型管理->消息类型用户',
			content: ['<%=request.getContextPath()%>/admin/messageType/update?id=' + id,'no']
		});
	}
	...
	function deleteType(id) {
		layer.confirm('是否确定此操作？', {
			...
			$.ajax({
				type: 'POST',
				url: '<%=request.getContextPath()%>/admin/messageType/delete',
				dataType: 'json',
				data: {
					id: id
				},
	...
	function editStatus(id, status){
		...
		layer.confirm('是否确定此操作？', {
			...
			$.ajax({
				url: '<%=request.getContextPath()%>/admin/messageType/updateStatus',
				dataType: 'json',
				type: 'POST',
				data: {
					id:id,
