...
<body>
	...
	function tableCallBack(data,id){
        if(id == 'button'){
            return '<button type="button" class="btn btn-primary btn-xs" onclick="edit(\'' + data.id + '\');">修改</button>';;
	...
	function edit(id){
		layer.open({
    	    type: 2,
    	    area: ['700px', '550px'],
    	    offset: [50,(($(window).width()-700)/2)],
    	    title: '修改',
    	    content: '<%=request.getContextPath()%>/admin/borrowProduct/editPage?id=' + id
    	});
    }
	...
	function add(){
		layer.open({
    	    ...
    	    offset: [50,(($(window).width()-700)/2)],
    	    title: '新增',
    	    content: '<%=request.getContextPath()%>/admin/borrowProduct/addPage'
    	});
	}
