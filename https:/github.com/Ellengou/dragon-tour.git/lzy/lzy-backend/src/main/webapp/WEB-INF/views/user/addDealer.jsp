										...
										<div class="row">
											...
											<div class="col-xs-3">
												<div class="input-group">
													...
													<input type="text" class="form-control" id="grLoginName" name="grLoginName">
												</div>
											</div>
										</div>
									</div>
	...
	$('.dataFile').each(function(){
		...
		function submit() {
			...
			if(jsonData.compType == '2'){
				...
				jsonData.loginName = jsonData.grLoginName;
			}
			...
			var ids = [];
			  ...
			  layer.confirm('确定保存?', {
		    	  btn: ['确定','取消'] //按钮
				    ...
				    layer.load(2);
				    $.ajax({
					      url: '<%=request.getContextPath()%>/admin/userFront/addDealer',
					      dataType: 'json',
					      type: 'POST',
					      data: jsonData,
					      success: function (data) {
				...
				$.ajax({
	  			      ...
	  			      success: function (data) {
	  			    	if(data.status == 'fail'){
		  			    	...
		  			    	//询问框
		  			      	layer.confirm('该经销商信息已存在，是否直接关联该经销商', {
		  			      	  btn: ['马上关联','取消'] //按钮
		  			      	}, function(){
		  			      		$.ajax({
