...
<script type="application/javascript">
    $(function () {
        $.ajax({
            type: 'GET',
            dataType: "json",
            url: '<%=request.getContextPath()%>/admin/region/user/json?userId=' + ${userId},
            success: function (data) {
                ...
                for(var i = 0; i < data.length; i++){
                    ...
                    if (data[i].checked == 1) {
                        template += '<input type="checkbox" name="role" id="' + data[i].id + '" checked>    ' + data[i].name;
                    } else {
                        template += '<input type="checkbox" name="role" id="' + data[i].id + '">    ' + data[i].name;
                    }
                    template += '</div>';
                }
                $('#main').html(template);
    ...
    function saveUserRole() {
        ...
        $.ajax({
            url: '<%=request.getContextPath()%>/admin/region/${userId}/updateUserRegion',
            type: 'POST',
            dataType: 'json',
