...
<body>
							...
							<div class="col-lg-12">
								<form id="myForm">
									...
									<div class="row">
										...
										<div class="col-xs-4">
											<div class="input-group">
												...
												<input type="text" class="form-control" id="bankCode" name="bankCode" value="${bank.bankCode}">
											</div>
										</div>
										<div class="col-xs-4">
											<div class="input-group">
												<span class="input-group-addon">图片名称</span> 
												<input type="text" class="form-control" id="imgName" name="imgName" value="${bank.imgName}">
											</div>
										</div>
									</div>
									<br>
									<div class="row">
										...
										<div class="col-xs-4">
											<div class="input-group">
												<span class="input-group-addon">本系统支付类型</span> 
												<input type="text" class="form-control" id="payClass" name="payClass" value="${bank.payClass}">
											</div>
										</div>
									</div>
									<hr>
	...
	<script>
		$(document).ready(function() {
			...
			var bankTypeList = $.parseJSON('${bankTypeList}');
			for (var i = 0; i < bankTypeList.length; i++) {
				$(new Option(bankTypeList[i].dicDataName,bankTypeList[i].dicDataValue)).appendTo('#type');
			}
			...
			$.validator.addMethod("bankCode",function(value,element,params){
				...
				if(value != null && value != ''){
					$.ajax({
					      url: '<%=request.getContextPath()%>/admin/bank/checkBankCode',
					      dataType: 'json',
					      type: 'POST',
					      async: false, 
					      data: {
		...
		function submit() {
			...
			$.ajax({
		      url: '<%=request.getContextPath()%>/admin/bank/editBank',
		      dataType: 'json',
		      type: 'POST',
		      data: jsonData,
		      success: function (data) {
