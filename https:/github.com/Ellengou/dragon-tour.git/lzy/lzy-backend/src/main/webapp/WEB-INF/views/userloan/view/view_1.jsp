...
<body>
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<div class="row">
							<div class="col-lg-12">
								<form id="myForm">
								<%@ include file="/WEB-INF/views/userloan/template/userLoanTemplate_1.jsp"%>
								<div id = "subPage">
									</div>
								<%@ include file="/WEB-INF/views/userloan/template/userSuppInfoTemplate.jsp"%>
									<ul id="myTab" class="nav nav-tabs">
										...
										<li><a href="#loanRecord">放款记录</a></li>
										<li><a href="#repayment">还款情况</a></li>
									</ul>
	...
	//单独激活每个标签
		var moneySource = '${loan.moneySource}';
		$('#myTab a').click(function (e) {
		  ...
		  if($this.attr('href') == "#verifySituation"){
			  ...
			  if(moneySource == '1'){
				  //平台发标
				  $.ajax({
					  url : '<%=request.getContextPath()%>/admin/userloan/getUserLoanFkRecord?id=${loan.id}',
					  type: 'GET',
					  success:function(data){
				...
				if(loanStatus == 'repaying' || loanStatus == 'complete'){
					var temp = '';
					temp += '<div class="panel panel-default">';
			  ...
			  if(moneySource == '1'){
				//平台发标
				  $.ajax({
					  url : '<%=request.getContextPath()%>/debtRepay/getLoansByUserLoanId?id=${loan.id}',
					  type: 'GET',
					  success:function(data){
						  $("#repayment").html(data);
					  }
				  ...
				  $.ajax({
					  url : '<%=request.getContextPath()%>/debtRepay/getUserLoanRepayInfo?id=${loan.id}',
					  type: 'GET',
					  success:function(data){
						  $("#repayment").html(data);
					  }
