	...
	$('.dataFile').each(function(){
		...
		$(document).ready(function() {
			...
			$('#lealIdCardValidId').datetimepicker({
				...
				if($('#lealIdCardValidDate').valid()){
					$('#lealIdCardValidDate').removeClass('invalid').addClass('success');
				};
			});
			...
			$('#businessTermId').datetimepicker({
				...
				if($('#businessTermDate').valid()){
					$('#businessTermDate').removeClass('invalid').addClass('success');
				};
			});
		...
		function submit() {
			...
			if(jsonData.compType == '2'){
			  ...
			  layer.load(2);
				  $.ajax({
  	  			      url: '<%=request.getContextPath()%>/admin/userFront/editSupplierAndDealer',
  	  			      dataType: 'json',
  	  			      type: 'POST',
  	  			      data: jsonData,
