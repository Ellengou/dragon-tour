								...
								<form id="myForm">
									...
									<div  class="form-group">
										...
										<div class="input-group col-xs-10 checkbox">
											<label>
											  <input type="checkbox" name="useType" value="0"> 经营性
											</label>
											<label>
											  <input type="checkbox" name="useType" value="1"> 固定资产
											</label>
										</div>
									</div>
									<div  class="form-group">
...
<script>
	...
	function save(){
		...
		$.ajax({
		      url: '<%=request.getContextPath()%>/admin/borrowProduct/update',
		      dataType: 'json',
		      type: 'POST',
		      data: jsonData,
	...
	$(document).ready(function(){
		...
		$("input[name = 'useType']").each(function(){
			var $this = $(this);
			var useType = '${product.useType}';
			if(useType.indexOf($this.val())>=0){
				$this.attr('checked',true);
			}
		})
		$("input[name = 'jkrdxs']").each(function(){
			var $this = $(this);
			var jkrdxs = '${product.userTypes}';
			if(jkrdxs.indexOf($this.val())>=0){
				$this.attr('checked',true);
			}
		})
		$("input[name = 'borrowTypes']").each(function(){
			var $this = $(this);
			var borrowTypes = '${product.borrowTypeIds}';
			if(borrowTypes.indexOf($this.val())>=0){
				$this.attr('checked',true);
			}
		})
