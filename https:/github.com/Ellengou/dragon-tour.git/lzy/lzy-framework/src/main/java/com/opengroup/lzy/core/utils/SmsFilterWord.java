 ...
 * @date 2015/6/17
 */
public class SmsFilterWord {

    static final Logger log = Logger.getLogger(SmsFilterWord.class);
    private final static File wordfilter = new File(SmsFilterWord.class.getClassLoader().getResource("config").getFile()+"/filterword.txt");
    ...
    private static void checkReload(){
        if(wordfilter.lastModified() > lastModified){
            synchronized(SmsFilterWord.class){
                try{
                    ...
                    while(lines.hasNext()){
                        ...
                        if(StringUtil.isNotBlank(line))
                            words.add(StringUtil.trim(line).toLowerCase());
                    }
                }catch(IOException e){
                    log.error("加载文件出错！",e);
                }
    ...
    public static void main(String[] args) throws Exception{
        System.out.println(SmsFilterWord.replace("中国共产党钓鱼岛 21世纪中国基金会 根据过滤字典过滤","*"));
        System.out.println(SmsFilterWord.isContain("做爱"));
        //BadWord.add("傻逼");
    }
