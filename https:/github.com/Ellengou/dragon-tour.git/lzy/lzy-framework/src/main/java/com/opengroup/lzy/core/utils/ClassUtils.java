...
public abstract class ClassUtils {
     ...
     * find the corresponding target method if there is one. E.g. the method may be <code>IFoo.bar()</code> and the
     * target class may be <code>DefaultFoo</code>. In this case, the method may be <code>DefaultFoo.bar()</code>. This
     ...
     * @param targetClass the target class for the current invocation. May be <code>null</code> or may not even
     * implement the method.
     * @return the specific target method, or the original method if the <code>targetClass</code> doesn't implement it
     ...
     * @return the Class array (<code>null</code> if the passed-in Collection was <code>null</code>)
     */
     ...
     * @return
     */
    public static Class<?> getSuperClassGenricType(final Class<?> clazz, final Integer index) {
        Type genType = clazz.getGenericSuperclass();

        if (!(genType instanceof ParameterizedType)) {
            Assert.isInstanceOf(genType.getClass(), genType, "'s superclass not ParameterizedType");
            return Object.class;
        }
        ...
        if (index >= params.length || index < 0) {
            Assert.hasLength("Index: " + index + ", Size of " + clazz.getSimpleName() + "'s Parameterized Type: "
                             + params.length);
            return Object.class;
        }
        if (!(params[index] instanceof Class)) {
            Assert.isInstanceOf(params[index].getClass(), params[index],
    ...
    public static Field[] getClassAllFields(final Class<?> clazz, final Class<?> endClazz) {
        Field[] fields = clazz.getDeclaredFields();
        Class<?> superClass = clazz.getSuperclass();
        Field[] superFields = null;
        while (superClass != null && superClass != endClazz) {
            superFields = superClass.getDeclaredFields();
            if (superFields.length > 0) {
                Field[] temp = fields.clone();
                fields = new Field[fields.length + superFields.length];
                System.arraycopy(temp, 0, fields, 0, temp.length);
                System.arraycopy(superFields, 0, fields, temp.length, superFields.length);
            }
    ...
    public static Field[] getClassAllFields(final Class<?> clazz) {
        return getClassAllFields(clazz, null);
    }
